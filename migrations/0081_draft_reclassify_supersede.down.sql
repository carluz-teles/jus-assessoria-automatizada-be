-- Revert 0081: restore the 0080 constraint exactly (task_id IS NOT NULL, no
-- superseded_at scoping).
--
-- CAVEAT — same one-way rollback risk 0080's down already documents, one level deeper:
...
-- the superseded ones, if the history is not worth keeping), then `migrate force 80` before
-- retrying. Do not roll back 0081 in an environment where the reclassify flow may already
