...
# cria via upsert, reconciliando (e adicionando as vars de billing/notifications que o
# provision.sh não tinha). O volume vem junto do serviço postgres (bloco nested).
#
# SEGURO: `terraform import` só ESCREVE O STATE LOCAL — não altera a infra na Railway.
# A mudança real só acontece no apply.sh (que você roda depois, revisando o plan).
set -euo pipefail
cd "$(dirname "$0")"
...
# Project id do court-legal (estável). Resolve os serviços a partir dele (não usa a query
# `me{}`, que é flaky). Override por env se algum dia mudar.
...
# Emite linhas "addr<TAB>id" pro import: postgres/redis são recursos próprios; os demais
# são railway_service.app["<nome>"].
...
[ -n "$MAP" ] || { log "Nenhum serviço encontrado — projeto certo? token certo?"; exit 1; }

# import idempotente: só se o endereço ainda não está no state.
imp() { # <addr> <id>
  local addr="$1" id="$2"
...
# 2) Serviços (postgres traz o volume junto)
while IFS=$'\t' read -r addr id; do
  [ -n "$addr" ] || continue
  imp "$addr" "$id"
done <<< "$MAP"

log "Import concluído. Agora rode: terraform plan  (revise os diffs — devem ser só as"
log "variable collections a criar + as vars novas de billing/notifications). Só então ./apply.sh."
