...
func TestEmbeddedSource(t *testing.T) {
	...
	// (chain continues below; 73 is the last — nothing follows it.)
	...
	if _, err := src.Next(73); !errors.Is(err, fs.ErrNotExist) {
		t.Fatalf("Next(73) error = %v, want fs.ErrNotExist", err)
	}
}
