...
func TestEmbeddedSource(t *testing.T) {
	...
	if next, err := src.Next(65); err != nil || next != 66 {
		t.Fatalf("Next(65) = (%d, %v), want (66, nil)", next, err)
	}
	if next, err := src.Next(66); err != nil || next != 67 {
		t.Fatalf("Next(66) = (%d, %v), want (67, nil)", next, err)
	}
	if next, err := src.Next(67); err != nil || next != 68 {
		t.Fatalf("Next(67) = (%d, %v), want (68, nil)", next, err)
	}
	if next, err := src.Next(68); err != nil || next != 69 {
		t.Fatalf("Next(68) = (%d, %v), want (69, nil)", next, err)
	}
	if next, err := src.Next(69); err != nil || next != 70 {
		t.Fatalf("Next(69) = (%d, %v), want (70, nil)", next, err)
	}
	...
	if next, err := src.Next(72); err != nil || next != 73 {
		t.Fatalf("Next(72) = (%d, %v), want (73, nil)", next, err)
	}
	if next, err := src.Next(73); err != nil || next != 74 {
		t.Fatalf("Next(73) = (%d, %v), want (74, nil)", next, err)
	}
	if next, err := src.Next(74); err != nil || next != 75 {
		t.Fatalf("Next(74) = (%d, %v), want (75, nil)", next, err)
	}
	...
	if next, err := src.Next(84); err != nil || next != 85 {
		t.Fatalf("Next(84) = (%d, %v), want (85, nil)", next, err)
	}
	if next, err := src.Next(85); err != nil || next != 86 {
		t.Fatalf("Next(85) = (%d, %v), want (86, nil)", next, err)
	}
	if next, err := src.Next(86); err != nil || next != 87 {
		t.Fatalf("Next(86) = (%d, %v), want (87, nil)", next, err)
	}
	if next, err := src.Next(87); err != nil || next != 88 {
		t.Fatalf("Next(87) = (%d, %v), want (88, nil)", next, err)
	}
	if next, err := src.Next(88); err != nil || next != 89 {
		t.Fatalf("Next(88) = (%d, %v), want (89, nil)", next, err)
	}
	if next, err := src.Next(89); err != nil || next != 90 {
		t.Fatalf("Next(89) = (%d, %v), want (90, nil)", next, err)
	}
	if next, err := src.Next(90); err != nil || next != 91 {
		t.Fatalf("Next(90) = (%d, %v), want (91, nil)", next, err)
	}
	if next, err := src.Next(91); err != nil || next != 92 {
		t.Fatalf("Next(91) = (%d, %v), want (92, nil)", next, err)
	}
	if next, err := src.Next(92); err != nil || next != 93 {
		t.Fatalf("Next(92) = (%d, %v), want (93, nil)", next, err)
	}
	if next, err := src.Next(93); err != nil || next != 94 {
		t.Fatalf("Next(93) = (%d, %v), want (94, nil)", next, err)
	}
	if next, err := src.Next(94); err != nil || next != 95 {
		t.Fatalf("Next(94) = (%d, %v), want (95, nil)", next, err)
	}
	if _, err := src.Next(95); !errors.Is(err, fs.ErrNotExist) {
		t.Fatalf("Next(95) error = %v, want fs.ErrNotExist", err)
	}
}
