...
func TestEmbeddedSource(t *testing.T) {
	...
	// 77 does not exist on this branch (it lands on a sibling branch not yet
	// merged here) — Next(76) legitimately skips straight to 78, the real next
	// file on disk; golang-migrate's Next() returns the next EXISTING version,
	// not "76+1".
	if next, err := src.Next(76); err != nil || next != 78 {
		t.Fatalf("Next(76) = (%d, %v), want (78, nil)", next, err)
	}
	if _, err := src.Next(78); !errors.Is(err, fs.ErrNotExist) {
		t.Fatalf("Next(78) error = %v, want fs.ErrNotExist", err)
	}
}
