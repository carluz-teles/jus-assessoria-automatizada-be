...
func TestEmbeddedSource(t *testing.T) {
	...
	// (chain continues below; 75 is the last — nothing follows it.)
	...
	if _, err := src.Next(75); !errors.Is(err, fs.ErrNotExist) {
		t.Fatalf("Next(75) error = %v, want fs.ErrNotExist", err)
	}
}
