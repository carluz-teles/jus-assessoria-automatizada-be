...
func TestEmbeddedSource(t *testing.T) {
	...
	// membership table); the billing slice adds 6 (the subscription table). So
	// 1→2→3→4→5→6 and 6 is the last — nothing follows it.
	...
	if _, err := src.Next(6); !errors.Is(err, fs.ErrNotExist) {
		t.Fatalf("Next(6) error = %v, want fs.ErrNotExist", err)
	}
}
