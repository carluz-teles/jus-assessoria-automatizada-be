...
func TestEmbeddedSource(t *testing.T) {
	...
	// (chain continues below; 69 is the last — nothing follows it.)
	...
	if _, err := src.Next(69); !errors.Is(err, fs.ErrNotExist) {
		t.Fatalf("Next(69) error = %v, want fs.ErrNotExist", err)
	}
}
