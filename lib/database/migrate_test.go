...
func TestEmbeddedSource(t *testing.T) {
	...
	// (chain continues below; 70 is the last — nothing follows it.)
	...
	if _, err := src.Next(70); !errors.Is(err, fs.ErrNotExist) {
		t.Fatalf("Next(70) error = %v, want fs.ErrNotExist", err)
	}
}
