...
func TestEmbeddedSource(t *testing.T) {
	...
	// (chain continues below; 74 is the last — nothing follows it.)
	...
	if _, err := src.Next(74); !errors.Is(err, fs.ErrNotExist) {
		t.Fatalf("Next(74) error = %v, want fs.ErrNotExist", err)
	}
}
