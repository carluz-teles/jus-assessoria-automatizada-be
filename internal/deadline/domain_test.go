...
type mockRepo struct {
	...
	// insertedDeadlineEvents accumulates EVERY InsertDeadlineEvent call, in order (multi-insert
	// creation flow — calculado + assumido — and batch flows like OnCourtRecordArchived, one
	// audit row per resolved prazo).
	insertedDeadlineEvents []*DeadlineEvent
...
// + assumido — and batch flows like OnCourtRecordArchived can insert one per resolved prazo).
...
func TestOnCourtRecordArchived_ResolvesPendingOpenAndMissed(t *testing.T) {
	...
	if len(repo.insertedDeadlineEvents) != 3 {
		t.Fatalf("deadline_event rows = %d, want 3 (one per resolved prazo)", len(repo.insertedDeadlineEvents))
	}
	for i, de := range repo.insertedDeadlineEvents {
...
func TestOnCourtRecordArchived_MarkResolvedGuardNoOp(t *testing.T) {
	...
	if len(repo.insertedDeadlineEvents) != 1 || repo.insertedDeadlineEvents[0].DeadlineID != okID {
		t.Errorf("deadline_event rows = %v, want exactly one for %q", repo.insertedDeadlineEvents, okID)
	}
