...
// call — that is what marks the event in the SAME transaction as the prazo it guards
// (mark + write commit atomically; a crash never strands a marked-but-unwritten event).
// Stateless; the worker injects the zero value.
type txDeduper struct{}

// NewDedup returns the deduper the use case marks consumed events with. Stateless — the
// tx is supplied per call — but a constructor keeps slice wiring uniform.
