...
type TaskView struct {
	...
	// Stage is the DERIVED 4-stage task stage — A_FAZER|ELABORACAO|REVISAO|CONCLUIDA. Populated
	// only by ListTasks (GET /v1/tasks): every task, peça-bound or not, sits in one of the four
	// stages there. ListTasksByProcesso does NOT set it (omitempty hides it from that response
	// instead of leaking a bogus ""). Set from the task's own status plus its vigente draft's
	// (LEFT JOIN, superseded_at IS NULL) hasDraft/sent_to_signing_at/filed_at.
...
// Stage constants — the closed set deriveTaskStage returns (GET /v1/tasks): EVERY task, peça-
// bound or not, sits in one of these four stages, moving A_FAZER → ELABORACAO → REVISAO →
// CONCLUIDA as work on it (and its optional draft) advances.
const (
	StageAFazer     = "A_FAZER"
	StageElaboracao = "ELABORACAO"
	StageRevisao    = "REVISAO"
	StageConcluida  = "CONCLUIDA"
)

// deriveTaskStage is the SINGLE source of truth for a task's 4-stage stage — the read repo
// calls it per row (ListTasks) off the task's own status plus its vigente draft (superseded_at
// IS NULL, draft_task_id_uidx guarantees at most one). Rules, evaluated in order:
//   - the task is DONE, OR its draft is already filed (filedAt != nil) → CONCLUIDA;
//   - it has a draft already sent to signing (sentToSigningAt != nil)  → REVISAO;
//   - it has a draft at all (not yet sent to signing)                 → ELABORACAO;
//   - otherwise (no draft)                                            → A_FAZER.
func deriveTaskStage(status TaskStatus, hasDraft bool, sentToSigningAt, filedAt *time.Time) string {
	if status == TaskStatusDone || filedAt != nil {
		return StageConcluida
	}
	if hasDraft && sentToSigningAt != nil {
		return StageRevisao
	}
	if hasDraft {
		return StageElaboracao
	}
	return StageAFazer
}
