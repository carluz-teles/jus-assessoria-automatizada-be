...
// TaskCreated announces one freshly created task from the manual POST /v1/tasks path. DueDate is
// the wire date (2006-01-02), omitted when the task has none; AssigneeUserID is omitted when
// unassigned. ActionItemID is empty here — the providência-driven task is now created
// SYNCHRONOUSLY by internal/actionitem via the ActionItemTaskCreator adapter (no event), so this
// event no longer carries the reverse-pointer role it did under the old async flow. The aggregate
// is the task, so its stream orders by the task id.
...
// newTaskCreatedFromTask builds the produced event from a persisted *Task — the single
// construction site for task.created, emitted by the manual CREATE path (POST /v1/tasks). The
// providência-driven task no longer emits it (creation is synchronous via ActionItemTaskCreator).
// aggregate_id is the task id (a uuid, satisfying the outbox's
