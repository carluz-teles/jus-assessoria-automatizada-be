...
func TestVoyageEmbedder_RequestAndResponse(t *testing.T) {
	...
	// Voyage is asymmetric — the indexing side must send input_type "document", the RAG query
	// side "query". Cover both explicit modes plus the empty-default (falls back to "document" so
	// a missed call site never silently mis-embeds a corpus batch).
	cases := []struct {
		name      string
		inputType InputType
		wantWire  string
	}{
		{name: "document side", inputType: InputDocument, wantWire: "document"},
		{name: "query side", inputType: InputQuery, wantWire: "query"},
		{name: "empty defaults to document", inputType: "", wantWire: "document"},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Point the adapter at the test server via a rewriting transport (the endpoint is a const).
			e := &VoyageEmbedder{apiKey: "sk-test", model: "voyage-3.5-lite", dim: 1024, client: &http.Client{Transport: rewriteTo(srv.URL)}}

			vecs, model, err := e.Embed(context.Background(), []string{"a", "b"}, tc.inputType)
			if err != nil {
			...
			if gotBody.Model != "voyage-3.5-lite" || gotBody.OutputDimension != 1024 || gotBody.InputType != tc.wantWire {
				t.Errorf("request body wrong: got input_type=%q want %q; %+v", gotBody.InputType, tc.wantWire, gotBody)
			}
...
func TestVoyageEmbedder_EmptyInputNoOp(t *testing.T) {
	...
	vecs, model, err := e.Embed(context.Background(), nil, InputDocument)
...
func TestVoyageEmbedder_Non2xxIsInfra(t *testing.T) {
	...
	e := &VoyageEmbedder{apiKey: "sk", model: "m", dim: 8, client: &http.Client{Transport: rewriteTo(srv.URL)}}
	_, _, err := e.Embed(context.Background(), []string{"x"}, InputQuery)
	if err == nil {
