...
WHERE i.tenant_id = $1
  ...
  AND (
    $6::text = ''
    OR ($6::text = 'atraso'         AND (d.end_date - CURRENT_DATE) < 0  AND d.status IN ('PENDING', 'OPEN'))
    OR ($6::text = 'hoje'           AND (d.end_date - CURRENT_DATE) = 0  AND d.status IN ('PENDING', 'OPEN'))
    OR ($6::text = 'semana'         AND (d.end_date - CURRENT_DATE) BETWEEN 1 AND 7 AND d.status IN ('PENDING', 'OPEN'))
    OR ($6::text = 'mais_adiante'   AND (d.end_date - CURRENT_DATE) > 7   AND d.status IN ('PENDING', 'OPEN'))
    OR ($6::text = 'nao_confirmado' AND d.status = 'PENDING')
  )
`
...
WHERE i.tenant_id = $1
  ...
  AND (
    ...
    OR ($7::text = 'semana'         AND (d.end_date - CURRENT_DATE) BETWEEN 1 AND 7 AND d.status IN ('PENDING', 'OPEN'))
  ...
  AND (i.made_available_at, i.id) < ($8::date, $9::uuid)
