...
-- name: AssignIntimationAssignee :one
...
-- name: BulkAssignIntimacoesByIDs :execrows
...
-- name: BulkAssignIntimacoesByFilter :execrows
 ...
 WHERE tenant_id = @tenant_id::uuid
   AND id IN (
     ...
     WHERE i.tenant_id = @tenant_id::uuid
       ...
       AND (
         sqlc.narg('filter_assignee_id')::uuid IS NULL
         OR i.assignee_user_id = sqlc.narg('filter_assignee_id')::uuid
       )
       AND (
         @urgencia::text = ''
